...
families <- (nematodes[,c(5,12:22)])



library(reshape2)
gg<-melt(families,id="tr_site")

familiesplot<- ggplot(gg, aes(x=variable, y=value, fill=factor(tr_site))) + 
...
theme( panel.background  = element_blank())+
  scale_fill_manual(values=c("powderblue" ,"hotpink1","dodgerblue","firebrick3"))+
  theme(axis.title.y = element_text(size=18),
        axis.text.y=element_text(size=14))+
  theme(axis.line = element_line(colour="black"))+
  labs(x = "Family", y= "Abundance (indv/g)",size=30)+
...
trophic_plot_puec<- ggplot(gg2, aes(x=variable, y=value, fill=factor(treatment))) + 
  ...
  coord_cartesian(ylim = c(0, 50), expand=c(0,0))+
  ggtitle("France.17 years of rainfall exclusion")+
  theme(plot.title=element_text(size=25))+
  theme(
    plot.title = element_text(hjust = 0.5))+
